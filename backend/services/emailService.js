...
const sendOtpEmail = async ({ name, email, otp }) => {
    ...
    if (!resend) {
        // Log OTP in dev when email is not configured
    ...
    return resend.emails.send({
        from: `${BRAND} <${FROM}>`,
        to: email,
        subject: `${otp} is your WAVWAY verification code`,
        html: base(`
          <h1 style="margin-bottom:6px;">Verify Your Email 📧</h1>
          <p>Hi <strong>${name}</strong>, enter the code below to complete your WAVWAY account registration. The code expires in <strong>10 minutes</strong>.</p>
          ...
          <div style="margin:36px auto;text-align:center;">
            <div style="display:inline-block;background:#0a0a0a;border-radius:16px;padding:28px 48px;">
              <p style="color:rgba(255,255,255,0.5);font-size:11px;text-transform:uppercase;letter-spacing:0.2em;margin:0 0 14px;">Your Verification Code</p>
          ...
          <div class="highlight-box" style="background:#fef9c3;border-color:#fde68a;margin-bottom:24px;">
            <p style="margin:0;font-size:13px;color:#92400e;font-weight:600;">⚠️ Keep this code private</p>
            <p style="margin:4px 0 0;font-size:12px;color:#b45309;">WAVWAY will never ask for this code over phone or chat. If you didn't request this, you can safely ignore this email.</p>
          </div>

          <div class="divider"></div>
          <p style="font-size:13px;color:#71717a;margin:0;">Didn't receive it? Check your spam folder or request a new code.</p>
        `)
    });
};
