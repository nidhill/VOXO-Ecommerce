...
router.post('/send-otp', async (req, res) => {
    try {
        ...
        const otp = await Otp.createOtp(email.toLowerCase().trim());
...
router.post('/register', validate(registerSchema), async (req, res) => {
    try {
        ...
        // Verify OTP
        const valid = await Otp.verifyOtp(email.toLowerCase().trim(), otp);
...
router.get('/me', protect, async (req, res) => {
    res.json({ id: req.user._id, name: req.user.name, email: req.user.email, phone: req.user.phone, googleId: req.user.googleId || null });
});
